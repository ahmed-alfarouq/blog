    ...
    <nav role="navigation" aria-label="{{ __('Pagination Navigation') }}"
        ...
        <div class="flex justify-between flex-1 sm:hidden">
            @if ($paginator->onFirstPage())
                <x-button class="w-36 disabled:hover:text-secondary" disabled>
                    {!! __('pagination.previous') !!}
                </x-button>
            @else
                <a href="{{ $paginator->previousPageUrl() }}"
                    class="w-36 px-6 py-1 text-lg font-medium text-center border border-primary-border rounded-sm text-secondary hover:text-white hover:bg-primary transition-all duration-300 cursor-pointer">
                    {!! __('pagination.previous') !!}
                </a>
            @endif
        ...
        <div class="hidden sm:flex sm:items-center relative z-0 rtl:flex-row-reverse mx-auto shadow-sm rounded-md">
            @if ($paginator->onFirstPage())
            ...
            @if ($paginator->hasMorePages())
                <a href="{{ $paginator->nextPageUrl() }}" rel="next"
                    class="relative inline-flex items-center px-2 py-2 -ml-px text-sm font-medium text-gray-500 bg-white border border-gray-300 rounded-r-md leading-5 hover:text-gray-400 focus:z-10 focus:outline-none ring-gray-300 active:bg-gray-100 active:text-gray-500 transition ease-in-out duration-150 dark:bg-gray-800 dark:border-gray-600 dark:active:bg-gray-700"
